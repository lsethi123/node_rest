...
mongoClient.open(function(err, mongoClient) {
    db = mongoClient.db("employeedb07");
...
exports.updateEmployee = function  (req,res) {
    var update_emp_oject= req.body;
    var emp_id = update_emp_oject['_id'];
    delete(update_emp_oject['_id']);
    //console.log('update_emp_oject',update_emp_oject)
    db.collection('employees',function(error,employee_collection){
        if(error) res.jsonp(error)
        else{
            employee_collection.update({_id: employee_collection.db.bson_serializer.ObjectID.createFromHexString(emp_id)},update_emp_oject,
             function(){
